...
public class SemWifiApMonitor {
    ...
    public void hostapdCallbackEvent(String iface, String tstr) {
        ...
        if (mstr[0].equals("WPS-OVERLAP-DETECTED")) {
            sendMessage(iface, WPS_OVERLAP_DETECTED, str);
        }
        if (mstr[0].equals("AP_STA_CONNECTED")) {
            sendMessage(iface, AP_STA_ASSOCIATION_EVENT, str);
        } else if (mstr[0].equals("AP-STA-REMOVE")) {
            sendMessage(iface, AP_STA_REMOVE_EVENT, str);
        } else if (mstr[0].equals("AP-STA-DEAUTH")) {
            sendMessage(iface, AP_STA_DEAUTH_EVENT, str);
        } else if (mstr[0].equals("AP-STA-DISASSOC")) {
            sendMessage(iface, AP_STA_DISASSOCIATION_EVENT, str);
        } else if (mstr[0].equals("CTRL-EVENT-DRIVER-STATE")) {
            sendMessage(iface, CTRL_EVENT_DRIVER_STATE_EVENT, str);
        } else if (mstr[0].equals("WPS-SUCCESS")) {
            sendMessage(iface, WPS_SUCCESS_EVENT, str);
        } else if (mstr[0].equals("AP-STA-POSSIBLE-PSK-MISMATCH")) {
            sendMessage(iface, AP_STA_POSSIBLE_PSK_MISMATCH_EVENT, str);
        } else if (mstr[0].equals("WPS_EVENT_FAIL")) {
            sendMessage(iface, WPS_FAIL_EVENT, str);
        } else if (mstr[0].equals("WPS-TIMEOUT")) {
            sendMessage(iface, WPS_TIMEOUT_EVENT, str);
        } else if (mstr[0].equals("WPS-PIN-NEEDED")) {
            sendMessage(iface, WPS_PIN_NEEDED_EVENT, str);
        } else if (mstr[0].equals("AP-CSA-FINISHED")) {
            sendMessage(iface, AP_CSA_FINISHED_EVENT, str);
        } else if (mstr[0].equals("AP-STA-NOTAllOW")) {
            sendMessage(iface, AP_STA_NOTALLOW_EVENT, str);
        } else if (mstr[0].equals("AP-STA-NOTIF-DISASSOC")) {
            sendMessage(iface, AP_STA_DISCONNECTED_EVENT, str);
        } else if (mstr[0].equals("AP-STA-NEW")) {
            sendMessage(iface, AP_STA_NEW_EVENT, str);
        }
    }
}
