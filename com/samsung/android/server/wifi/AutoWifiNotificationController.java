...
public class AutoWifiNotificationController {
    ...
    private String mCurrentAddedNotiSsid;
    private String mCurrentAddedNotiTag;
    private String mCurrentDeletedNotiSsid;
    ...
    public AutoWifiNotificationController(Context context, AutoWifiController.AutoWifiAdapter adapter, List<String> exceptNetworks) {
        ...
        this.mBroadcastReceiver = new BroadcastReceiver() {
            /* class com.samsung.android.server.wifi.AutoWifiNotificationController.C07161 */
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (AutoWifiNotificationController.ACTION_AUTOWIFI_NOTIFICATION_SETTINGS.equals(intent.getAction())) {
                    ...
                    if (AutoWifiNotificationController.this.mCurrentAddedNotiSsid != null) {
                        ...
                        AutoWifiNotificationController.this.mCurrentAddedNotiSsid = null;
                    }
                    ...
                    if (notiId == AutoWifiNotificationController.this.mNotiIcon) {
                        ...
                        AutoWifiNotificationController.this.mCurrentDeletedNotiSsid = null;
                    }
    ...
    public synchronized void setNetworkState(boolean isConnected, boolean isWifiOffByUser, WifiConfiguration config, String networkKey) {
        if (isConnected) {
            if (checkApReachToScore(config)) {
                if (!this.mEnableToUse) {
                    Log.d(TAG, "AutoWifi is disabled, do not add to Favorite APs");
                    return;
                }
                setAndShowNotification(config, networkKey, true);
            }
            if (this.mLastNetworkConfig == null) {
                this.mConnectedTime = Calendar.getInstance().getTimeInMillis();
                this.mLastNetworkConfig = config;
                this.mLastNetworkKey = networkKey;
            }
        } else if (this.mLastNetworkConfig != null) {
            if (isWifiOffByUser) {
                checkAndDecreaseScore();
            }
            if (this.mLastNetworkConfig.semAutoWifiScore < 4 || this.mLastNetworkConfig.semAutoWifiScore == 99) {
                checkAndIncreaseScore();
            }
            this.mLastNetworkConfig = null;
            this.isApWithNumerousCellId = false;
        }
    }
