...
public class WifiControlHistoryProvider extends ContentProvider {
    ...
    public static void setControlHistory(Context context, String packageName, boolean enable) {
        ...
        if (packageName == null || packageName.isEmpty()) {
            ...
            return;
        } else if (!"system".equals(packageName) && !"android".equals(packageName) && !packageName.startsWith("state_change_warning.") && !packageName.startsWith(FACTORY_RESET)) {
            int count = 0;
            int enableNumber = 0;
            int disableNumber = 0;
            if (enable) {
            ...
            try {
                try {
                    cursor = context.getContentResolver().query(uri, null, "(package_name == '" + packageName + "')", null, null);
                    if (cursor != null) {
                        try {
                        ...
                        if (count > 0) {
                            ...
                            if (context.getContentResolver().update(uri, contentValues, null, new String[]{packageName}) <= 0) {
    ...
    public static void dumpControlHistory(Context context, PrintWriter pw) {
        Cursor cursor = null;
        try {
            cursor = context.getContentResolver().query(Uri.parse(URI_STRING), null, null, null, "time_stamp ASC");
            if (cursor == null) {
            ...
            while (!cursor.isAfterLast()) {
                ...
                sb.append((Object) lastControlTime);
