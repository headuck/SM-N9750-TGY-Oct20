...
public class WifiB2BConfigurationPolicy {
    ...
    public B2BConfiguration getConfiguration(String ssid) {
        synchronized (this.mConfigLock) {
            ...
            return this.mWiFiB2BConfPolicy.get(ssid);
        }
    }
    ...
    private void enforceEDMConfPermission() {
        ...
        if (context != null && (edm = EnterpriseDeviceManager.getInstance(context)) != null) {
            edm.enforceActiveAdminPermission(new ArrayList(Arrays.asList("com.samsung.android.knox.permission.KNOX_DEVICE_CONFIGURATION")));
        }
    }
                    ...
                    if (wifiConfigs.length != 0) {
                        ...
                        for (Parcelable wifiConfig : wifiConfigs) {
                            ...
                            if (result2 == 0) {
                                ...
                                conf.roamTrigger = roamThreshold;
                                conf.roamDelta = roamDelta;
                                conf.roamScanPeriod = roamScanPeriod;
                                conf.noDHCPinRoam = ((Bundle) wifiConfig).getBoolean(CONF_KEY_ROAM_NO_DHCP_RENEWAL, false);
                                conf.noBlackList = ((Bundle) wifiConfig).getBoolean(CONF_KEY_NO_CONN_BLACKLIST, false);
    ...
    public static class B2BConfiguration {
        private boolean noBlackList;
        private boolean noDHCPinRoam;
        private int roamDelta;
        private int roamScanPeriod;
        private int roamTrigger;
