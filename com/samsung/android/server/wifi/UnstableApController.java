...
public class UnstableApController {
    ...
    public boolean isUnstableAp(String bssid) {
        ...
        for (Integer num : this.mUnstableAps.keySet()) {
            UnstableApInfo unstableApInfo = this.mUnstableAps.get(Integer.valueOf(num.intValue()));
    ...
    public boolean disconnectWithAuthFail(int networkId, String bssid, int rssi, int disableReason, boolean isConnectedState, boolean isHotspotAp) {
        ...
        boolean ret = this.mDeauthAttackDetector.checkDeauthAttack(config, bssid, rssi);
        if (ret && config != null) {
            disconnect(bssid, -50, config, 0, false);
        }
        if (!isConnectedState && isHotspotAp) {
            return disconnect(bssid, -50, config, 0, false) || ret;
        }
        String str = this.mLastAuthFailBssid;
        if (str == null || !str.equals(bssid)) {
            this.mLastAuthFailBssid = bssid;
        ...
        if (!ret && this.mAuthFailCounter >= 3) {
            ...
            return disconnect(bssid, rssi, config, disableReason, true) || ret;
        } else if (config == null || (!isKoreaVendorAp(config) && !isVendorPasspointAp(config))) {
            return ret;
        } else {
            return disconnect(bssid, rssi, config, 0, true) || ret;
        }
    }
    ...
    public static class DeauthAttackDetector {
    ...
    /* access modifiers changed from: private */
    public static class UnstableApInfo {
