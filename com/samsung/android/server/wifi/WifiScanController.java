...
public class WifiScanController {
    ...
    private int getCustomScanType(String packageName) {
        ...
        synchronized (this.mNLPPackages) {
            ...
            return scanSetting.mScanType;
        }
    }

    private boolean checkScanDelayForCachedScan(String packageName) {
        String key = getNLPPackageKey(packageName);
        if (key == null) {
            return false;
        }
        synchronized (this.mNLPPackages) {
            NLPScanSettings scanSetting = this.mNLPPackages.get(key);
            if (scanSetting == null || scanSetting.mScanDelayMillis == 0 || SystemClock.elapsedRealtime() - this.mLastActualScanActionTime >= scanSetting.mScanDelayMillis) {
                return false;
            }
            return true;
        }
    ...
    public static class NLPScanSettings {
