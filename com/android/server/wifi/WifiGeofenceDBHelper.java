...
public class WifiGeofenceDBHelper extends SQLiteOpenHelper {
    ...
    public void insert(int locationID, int networkID, String configKey, String BSSID, long time) {
        ...
        data.put(KEY_CONFIG_KEY, configKey);
        data.put("bssid", BSSID);
        data.put("time", (Long) 0L);
        data.put(KEY_LATITUDE, (Integer) 0);
        data.put(KEY_LONGITUDE, (Integer) 0);
        data.put(KEY_TIME_MAJOR, Long.valueOf(time));
        data.put(KEY_LATITUDE_MAJOR, (Long) 1000L);
        data.put(KEY_LONGITUDE_MAJOR, (Long) 1000L);
        db.insert(TABLE_NAME, null, data);
        Log.d(TAG, "insert() - locationID : " + locationID + ", configKey : " + configKey + ", networkID : " + networkID + ", BSSID : " + BSSID + ", time : " + time);
        this.mDataList.put(new Integer(locationID), new WifiGeofenceData(locationID, networkID, configKey, BSSID, time, 1000.0d, 1000.0d));
    }
    ...
    public void update(int locationID, long time) {
        ...
        if (find(locationID) != 0) {
            ContentValues data = new ContentValues();
            data.put(KEY_TIME_MAJOR, Long.valueOf(time));
            db.update(TABLE_NAME, data, "location_id=" + locationID, null);
    ...
    public void delete(String configKey) {
        ...
        if (locationId != -1) {
            db.delete(TABLE_NAME, "location_id=" + locationId, null);
    ...
    private int getLocationId(String configKey) {
        int locationId = -1;
        Cursor cursor = getReadableDatabase().rawQuery("select * from geofence_wifi", null);
        if (cursor != null) {
            while (true) {
    ...
    public HashMap<Integer, WifiGeofenceData> select() {
        ...
        Cursor cursor = db.rawQuery("select * from geofence_wifi", null);
        if (cursor != null) {
            while (cursor.moveToNext()) {
                int _locationid = cursor.getInt(1);
                ...
                this.mDataList.put(new Integer(_locationid), new WifiGeofenceData(_locationid, _networkid, _configKey, _bssid, _time, _latitude, _longitude));
                Log.e(TAG, "WifiGeofenceDBHelper select - *" + _locationid + "*" + _networkid + "*" + _configKey + "*" + _bssid + "*" + _time + "*" + isValidLocation(_latitude, _longitude));
                db = db;
                cursor = cursor;
            }
            cursor.close();
    ...
    public int getLocationIdFromOldTime() {
        int locationId = -1;
        Cursor cursor = getReadableDatabase().rawQuery("select location_id from geofence_wifi order by time", null);
        if (cursor != null) {
            if (cursor.moveToNext()) {
