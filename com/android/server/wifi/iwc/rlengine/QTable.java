...
public class QTable {
    ...
    public QTable(float[] param, IWCLogFile logFile) {
        ...
        this.qTable = (float[][]) Array.newInstance(float.class, this.numStates, this.numActions);
        this.IWCLog = logFile;
    }
    ...
    private boolean checkSteadyState() {
        ...
        if (this.eventBuffer.size() > 0) {
            ...
            writeLog(longestStreak, aStreak, this.STEADSTATETHRESHOLD, this.eventBuffer.size(), this.eventBufferLimit, false, this.isSteadyState, this.firstIndexToggling, this.movedFirstIndexToggling, this.steakTogglingCnt, this.zeroIndexReached, "variable info");
            int i = this.STEADSTATETHRESHOLD;
            if (longestStreak < i - 1) {
                y = aStreak;
                if (((float) longestStreak) + (((float) y) / 4.0f) < ((float) i)) {
                    if (((float) numMinLM) / ((float) numMaxLM) > 0.25f || numMinLM <= 0 || numMaxLM < i) {
    ...
    public int addEvent(boolean snsFlag, boolean aggFlag, RewardEvent eventType) {
        ...
        checkConsSNSToggling(eventType);
