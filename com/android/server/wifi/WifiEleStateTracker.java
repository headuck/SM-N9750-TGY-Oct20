...
public class WifiEleStateTracker {
    ...
    public WifiEleStateTracker(Context context, WifiNative wifiNative, WifiConnectivityMonitor wificonnectivityMonitor) {
        ...
        this.mEleGeoMagneticAction = new WifiEleGeoMagnetic.OnEleDetectInterface() {
            /* class com.android.server.wifi.WifiEleStateTracker.C04541 */

            @Override // com.android.server.wifi.WifiEleGeoMagnetic.OnEleDetectInterface
    ...
    private void runEleBlockRoamTimer() {
        ...
        this.mEleBlockRoamTimer.schedule(new TimerTask() {
            /* class com.android.server.wifi.WifiEleStateTracker.C04552 */

            public void run() {
                Log.i(WifiEleStateTracker.TAG, "mEleBlockRoamTimer timer expired - enable Roam network valid transition");
                WifiEleStateTracker.this.mEleBlockRoamTimer = null;
                if (WifiEleStateTracker.this.mEleBlockRoamConnection) {
                    ...
                    WifiEleStateTracker.this.mEleBlockRoamConnection = false;
                }
            }
    ...
    private int checkEleEnvironmentConfirmFactors(int bcnDiff, int mobileRssi, int wifiRssi, int previousNonZeroBeaconCnt, int previousStableBeaconCnt, int prevAverBcnCnt, boolean txbadIncrease) {
        ...
        if (bcnDiff == 0 && bStationaryForFiveSeconds) {
            Log.d(TAG, "CheckEleEnvironment - bcnDiff Zero! mEleExpireCount : " + this.mEleExpireCount + " wifiRssi : " + wifiRssi + " mElePrevWifiRssi[0] : " + this.mElePrevWifiRssi[0] + " previousNonZeroBeaconCnt : " + previousNonZeroBeaconCnt + " previousStableBeaconCnt : " + previousStableBeaconCnt + " prevAverBcnCnt : " + prevAverBcnCnt);
        ...
        int i = this.mEleBcnCheckingState;
        if (i != 2 && i != 1) {
            return 0;
        }
        Log.i(TAG, "CheckEleEnvironment - Beacon loss checking starteded!");
        if ((!this.mGeomagneticEleState || !checkGeoMagneticRecentChange()) && !this.mEleDetectionPending && ((this.mEleBigSignalChangeExpirationCnt <= 0 || !bStationaryForFiveSeconds) && !getMobileRssiChangeWithTimeDiff(mobileRssi, 3, 5) && !getMobileRssiChangeWithTimeDiff(mobileRssi, 4, 6) && ((!getMobileRssiChangeWithTimeDiff(mobileRssi, 6, 8) || wifiRssi >= this.mElePrevWifiRssi[0]) && ((!checkContiousMobileRssiDecrease(mobileRssi) || !getMobileRssiChangeWithTimeDiff(mobileRssi, 2, 3) || !getWifiRssiChangeWithTimeDiff(wifiRssi, 2, 5)) && (!getMobileRssiChangeWithTimeDiff(mobileRssi, 6, 11) || wifiRssi > this.mElePrevWifiRssi[0]))))) {
            return 0;
        }
    ...
    public int checkEleEnvironment(int mobileRssi, int runningBcnCount, int wifiRssi, int txBad) {
        ...
        int bcnDiff2;
        int retVal = 0;
        int i = this.mEleExpireCount;
        if (i % 6 == 0 || i == 1) {
            printEleFactorsHistory();
        }
        int i2 = this.mEleStepExpirationCnt;
        if (i2 > 0) {
            this.mEleStepExpirationCnt = i2 - 1;
        }
        int i3 = this.mEleExpireCount;
        this.mEleExpireCount = i3 - 1;
        if (i3 == 0) {
        ...
        int i4 = this.mElePrevBcnCnt;
        if (i4 == -1) {
            bcnDiff = -1;
        } else if (runningBcnCount < 0) {
            resetEleParameters(1, false, true);
            return 0;
        } else {
            int bcnDiff3 = runningBcnCount - i4;
            if (bcnDiff3 <= -1) {
        ...
        this.mElePrevTxBadCnt = txBad;
        if (wifiRssi > ELE_MINIMUM_RSSI || this.mEleBcnHistoryCnt < 6 || bcnDiff >= 4 || wifiRssi > this.mElePrevWifiRssi[0]) {
            bcnDiff2 = bcnDiff;
        } else {
            bcnDiff2 = bcnDiff;
        ...
        this.mElePrevBcnCnt = runningBcnCount;
        this.mPrevGeomagneticEleState = this.mGeomagneticEleState;
        int i6 = this.mEleStableCount;
        if (i6 != 0) {
            this.mEleStableCount = i6 - 1;
        }
        int i7 = this.mEleInvalidByLegacyDelayCheckCnt;
        if (i7 != 0) {
            this.mEleInvalidByLegacyDelayCheckCnt = i7 - 1;
        }
        int i8 = this.mEleBigSignalChangeExpirationCnt;
        if (i8 != 0) {
            this.mEleBigSignalChangeExpirationCnt = i8 - 1;
        }
        int i9 = this.mEleTwoBigSignalChangeExpirationCnt;
        if (i9 != 0) {
            this.mEleTwoBigSignalChangeExpirationCnt = i9 - 1;
        }
        this.mEleIsStepPending = false;
        return retVal;
    }
