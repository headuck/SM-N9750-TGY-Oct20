...
public class PasspointProvider {
    ...
    private void buildEnterpriseConfigForUserCredential(WifiEnterpriseConfig config, Credential.UserCredential credential) {
        ...
        boolean z = false;
        try {
            password = new String(Base64.decode(credential.getPassword(), 0), StandardCharsets.UTF_8);
        } catch (IllegalArgumentException e) {
            Log.e(TAG, "buildEnterpriseConfigForUserCredential, IllegalArgumentException bad base-64 password");
        ...
        List<String> list = this.mCaCertificateAliases;
        if (list != null) {
            config.setCaCertificateAliases((String[]) list.toArray(new String[0]));
        }
        int phase2Method = 0;
