...
public abstract class NetworkListStoreData implements WifiConfigStore.StoreData {
    ...
    private List<WifiConfiguration> parseNetworkList(XmlPullParser in, int outerTagDepth) throws XmlPullParserException, IOException {
        ...
        HashMap<String, WifiConfiguration> networks = new HashMap<>();
            ...
            try {
                ...
                if (config.allowedKeyManagement.get(1) || config.allowedKeyManagement.get(6)) {
                    if (config.preSharedKey.length() == 0) {
                        Log.e(TAG, "illegal network detected " + config.configKey());
                    } else if (config.allowedKeyManagement.cardinality() > 1 && config.allowedKeyManagement.get(6)) {
                        config.allowedKeyManagement = new BitSet(64);
                        config.allowedKeyManagement.set(1);
                        Log.e(TAG, "illegal network detected, but recovery " + config.configKey());
                    }
                }
                if (config.allowedKeyManagement.get(0)) {
                    if (config.wepKeys[0] != null) {
                        if (config.wepKeys[0].length() == 0) {
                            ...
                            for (int i = 1; i < config.wepKeys.length; i++) {
                                if (config.wepKeys[i] != null && config.wepKeys[i].length() == 0) {
                                    config.wepKeys[i] = null;
                                }
                            }
                        }
                    } else if (config.requirePMF) {
                        config.allowedKeyManagement = new BitSet(64);
                        config.allowedKeyManagement.set(9);
                    }
                }
                if (networks.get(config.configKey()) == null) {
                    networks.put(config.configKey(), config);
                } else {
                    Log.i(TAG, "duplicated network detected " + config.configKey());
                }
            } catch (RuntimeException e) {
                Log.e(TAG, "Failed to parse network config. Skipping...", e);
            }
        }
        networkList.addAll(networks.values());
        return networkList;
    }
