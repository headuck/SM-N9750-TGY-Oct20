...
public class ScanResultUtil {
    ...
    public static void dumpScanResults(PrintWriter pw, List<ScanResult> scanResults, long nowMs) {
        ...
        if (!(scanResults == null || scanResults.size() == 0)) {
            pw.println("    BSSID              Frequency      RSSI      BSS Load Element       Age(sec)     SSID                                 Flags");
            for (ScanResult r : scanResults) {
                ...
                pw.printf("  %17s  %9d  %18s %10s  %15s    %-32s  %s", r.BSSID, Integer.valueOf(r.frequency), rssiInfo, r.semBssLoadElement, age, String.format("%1.32s", ssid), r.capabilities);
                if ((r.flags & 1) != 0) {
                    hs20 = "[HS20]";
                }
                pw.printf("%s\n", hs20);
