    ...
    public static class Capabilities {
            ...
            try {
                if (buf.getShort() == 1) {
                    ...
                    for (int i2 = 0; i2 < akmCount; i2++) {
                        switch (buf.getInt()) {
                            case RSN_AKM_CCKM /*{ENCODED_INT: 9846784}*/:
        ...
        private void parseWapiElement(ScanResult.InformationElement ie) {
            ...
            try {
                ...
                this.protocol.add(4);
                short akmCount = buf.getShort();
                Log.i(InformationElementUtil.TAG, "parseWapiElement() akmCount : " + ((int) akmCount));
                ArrayList<Integer> rsnKeyManagement = new ArrayList<>();
                        ...
                        try {
                            ...
                            if ((16777215 & framePrefix) == 3282432) {
                                ...
                                if (tempVsData.length > 0) {
                                    this.mKtVsData = "VSD[2] = " + String.format("%02x", Byte.valueOf(tempVsData[0])) + ", ";
                                    if (tempVsData.length > 24) {
                                        this.mKtVsData += "VSD[26] = " + String.format("%02x", Byte.valueOf(tempVsData[24])) + " ";
                                    }
                                }
        ...
        public String generateCapabilitiesString() {
            ...
            if (!(b == 0 || this.semVsData == null)) {
                capabilities.append(String.format("[SEC%02x]", Byte.valueOf(b)));
                capabilities.append("[SECD");
                ByteBuffer bbuf = ByteBuffer.wrap(this.semVsData).order(ByteOrder.BIG_ENDIAN);
                for (int i2 = 0; i2 < this.semVsData.length; i2++) {
                    try {
                        capabilities.append(String.format("%02x", Byte.valueOf(bbuf.get())));
                    } catch (BufferUnderflowException e) {
                    }
                }
                capabilities.append("]");
