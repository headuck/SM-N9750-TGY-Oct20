...
public class TelephonyUtil {
    ...
    /* JADX INFO: Multiple debug info for r0v13 int: [D('kcOffset' int), D('rand' byte[])] */
    private static String getGsmAuthResponseWithLength(String[] requestData, TelephonyManager tm, int appType) {
        ...
        while (i2 < length) {
            ...
            if (challenge == null) {
                ...
                try {
                    String tmResponse = defaultDataTm.getIccAuthentication(appType, 128, Base64.encodeToString(parseHex(challenge), 2));
    ...
    public static SimAuthResponseData get3GAuthResponse(SimAuthRequestData requestData, TelephonyManager tm) {
        ...
        if (!(rand == null || authn == null)) {
            ...
            if (tm != null) {
                tmResponse = tm.createForSubscriptionId(mSubscriptionId).getIccAuthentication(2, NetworkConstants.ICMPV6_ECHO_REPLY_TYPE, base64Challenge);
                Log.v(TAG, "Raw Response - " + tmResponse);
            } else {
                Log.e(TAG, "No valid TelephonyManager");
            }
