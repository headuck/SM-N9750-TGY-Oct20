...
public class ScanDetail {
    ...
    public ScanDetail(NetworkDetail networkDetail, WifiSsid wifiSsid, String bssid, String caps, int level, int frequency, long tsf, ScanResult.InformationElement[] informationElements, List<String> anqpLines) {
        ...
        if (vsOuiType != 15 || vsData == null) {
            ...
            sb.append(String.format("%02X", Byte.valueOf(vsOuiType)));
            ByteBuffer bbuf = ByteBuffer.wrap(vsData).order(ByteOrder.BIG_ENDIAN);
            for (int i = 0; i < vsData.length; i++) {
                try {
                    sb.append(String.format("%02X", Byte.valueOf(bbuf.get())));
        ...
        this.mScanResult.semBssLoadElement = String.format("%02X%02X", Byte.valueOf(le_sc[0]), Byte.valueOf(le_sc[1])) + String.format("%02X", Byte.valueOf(channelUtilization)) + String.format("%02X%02X", Byte.valueOf(le_cap[0]), Byte.valueOf(le_cap[1]));
    }
